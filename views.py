...
def index_admin(request):
    ...
    totaldeliveredgdonations=Donation.objects.filter(status="Donation Delivered Succeessfully").count()    
    totaldonationreas=DonationArea.objects.all().count()    
...
def donationdelivered_admin(request):
    if not request.user.is_authenticated:
        return redirect('/login-admin')
    donation=Donation.objects.filter(status='Donation Delivered Succeesfully')        
...
class edit_area(View):
    ...
    def post(self, request, pid):
        ...
        if not request.user.is_authenticated:
            return redirect('/login-admin')
        form= DonationAreaForm(request.POST)
        area= DonationAre.objects.get(id=pid)
        areaname=request.POST['areaname']      
...
class accepted_donationdetail(View):
    def get(self,request, pid):
        donation=Doantion.objects.get(id=pid)
        donationarea=DoantionArea.objects.all()
        volunteer=Volunteer.objects.filter(status='accept')
...
class view_volunteerdetail(View):
    ...
    def post(self,request, pid):
        ...
        return render(request, "view-volunteerdetail", locals()) 
...
def view_donordetail(request, pid):
        if not request.user.is_authenticated:
            return redirect ('/login-admin') 
        donor=Donation.objects.get(id=pid)   
...
class view_donationdetail(View):
    ...
    def post(self,request, pid):
        ...
        try:
            ...
            messages.warning(request, 'Failed to update remark and status')
...
from django.contrib import messages
from .forms import DonateNowForm
...
class donate_now(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            try:
                Donation.objects.create(donor=donor, donationname=donationname, donationpic=donationpic, collectionloc=collectionloc, description=desription,
...
class  profile_donor(View):
    ...
    def post(self, request):
        ...
        try:
           ...
           messages.warning(request, 'Profile Updation Failed'+e) 
        return render(request, "profile-donor.html", locals())                
...
def index_volunteer(request):
    ...
    totalRecDonation=Donation.objects.filter(volunteer=volunteer, status="Donation Recived").count()
...
class donationrec_detail(View):
    def get(self,request,pid ):
        if not request.user.is_authenticated:
            return redirect('/login-admin')
        donation=Donation.objects.get(id=pid)  
        return render(request, "donationrec-detail.html", local())  
    def post(self,request, pid):
        if not request.user.is_authenticated:
            return redirect ('/login-admin')
        donation=Donation.objects.get(id=pid)  
        status=request.POST['status']
        deliverypic=request.POST['deliverypic']
        try:   
            donation.status=staus
            donation.updationdate=date.today()
            donation.save()
